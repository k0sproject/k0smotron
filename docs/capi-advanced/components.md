...
k0smotron creates the Kubernetes resources (for example `StatefulSet`, `Service`, and `ConfigMap`) through a `k0smotron.io/v1beta1` Cluster. You can customize those generated resources by defining patches.
...
After the cluster is provisioned, verify that the patches were applied.
...
- Only resources created and managed by the K0smotron Cluster are patch targets. Other resources are not; for example, when using Cluster API, certificate Secrets are created by Cluster API controllers and follow Cluster API naming conventions, so they are not patch targets.
