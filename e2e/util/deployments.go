...
func WaitForDeploymentsAvailable(ctx context.Context, input framework.WaitForDeploymentsAvailableInput, intervals ...interface{}) error {
	fmt.Printf("Waiting for deployment %s to be available", klog.KObj(input.Deployment))
	deployment := &appsv1.Deployment{}
	err := wait.PollUntilContextTimeout(ctx, time.Second*10, time.Minute*3, true, func(ctx context.Context) (done bool, err error) {
