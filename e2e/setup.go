...
func setup() error {
	...
	if hasK0smotronProvider(e2eConfig) {
		...
		err := setupPoolMachinesConfigEnv(ctx, replicas, e2eConfig.MustGetVariable(KubernetesVersion), e2eConfig)
		if err != nil {
	...
	// TODO: Implement framework.ClusterLogCollector for k0smotron infra provider.
	bootstrapClusterProxy = capiframework.NewClusterProxy("bootstrap", kubeconfigPath, scheme)
