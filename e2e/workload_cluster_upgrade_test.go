...
func workloadClusterUpgradeSpec(t *testing.T) {
	...
	clusterClassNamespace, _ := framework.CreateNamespaceAndWatchEvents(ctx, framework.CreateNamespaceAndWatchEventsInput{
	...
	clusterWithCCTemplate := clusterctl.ConfigCluster(ctx, clusterctl.ConfigClusterInput{
		...
		ClusterctlVariables: map[string]string{
			...
			"UPDATE_STRATEGY":        strings.TrimSuffix(flavor, "CAPI"),
		},
	})
