...
func newEnvironment(setupSecretCachingClient setupSecretCachingClientFn) *Environment {
	...
	crdPaths := []string{
		capiCoreCrdsPath,
		filepath.Join(root, "config", "clusterapi", "bootstrap", "crd", "bases"),
		filepath.Join(root, "config", "clusterapi", "controlplane", "crd", "bases"),
		filepath.Join(root, "config", "clusterapi", "infrastructure", "crd", "bases"),
	}
