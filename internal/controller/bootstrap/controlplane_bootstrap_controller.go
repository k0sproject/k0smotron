...
func (c *ControlPlaneController) genInitialControlPlaneFiles(ctx context.Context, scope *Scope, files []cloudinit.File) ([]cloudinit.File, error) {
	...
	err = c.createKubeconfig(ctx, scope)
	if err != nil {
		log.Error(err, "Failed to create kubeconfig")
		//return nil, err
	}
	log.Info("Kubeconfig created")

	return files, err
}

func (c *ControlPlaneController) genControlPlaneJoinFiles(ctx context.Context, scope *Scope, files []cloudinit.File) ([]cloudinit.File, error) {
	log := log.FromContext(ctx).WithValues("K0sControllerConfig cluster", scope.Cluster.Name)
...
func (c *ControlPlaneController) createKubeconfig(ctx context.Context, scope *Scope) error {
	if scope.Cluster.Spec.ControlPlaneEndpoint.IsZero() {
		return errors.New("control plane endpoint is not set")
	}

	return kubeconfig.CreateSecret(ctx, c.Client, scope.Cluster)
}
