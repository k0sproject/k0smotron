...
func TestReconcileControllerConfigControlPlaneIsZero(t *testing.T) {
	...
	require.EventuallyWithT(t, func(c *assert.CollectT) {
		...
		assert.True(c, conditions.IsFalse(updatedK0sControllerConfig, clusterv2.ReadyCondition))
		...
		assert.Equal(c, clusterv2.ConditionSeverityInfo, *conditions.GetSeverity(updatedK0sControllerConfig, bootstrapv1.DataSecretAvailableCondition))
	}, 10*time.Second, 100*time.Millisecond)
}
