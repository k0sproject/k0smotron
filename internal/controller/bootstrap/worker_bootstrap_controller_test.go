	...
	tests := []struct {
	...
	}{
		{
			...
			want: []string{
				...
				"powershell.exe -NoProfile -NonInteractive -Command \"& { powershell.exe -NoProfile -NonInteractive -Command \"& { k0s.exe install worker --token-file /etc/k0s.token }\" --labels=k0smotron.io/machine-name= --kubelet-extra-args=\"--hostname-override=\" }",
