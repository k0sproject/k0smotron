...
func (scope *kmcScope) generateStatefulSet(ctx context.Context, kmc *km.Cluster) (apps.StatefulSet, apps.StatefulSet, error) {
	...
	volumeName, volumeMountPath, volumeNeeded := k0sDataVolume(kmc)
	...
	case "emptyDir":
		fallthrough
	default:
		// Only needed to share the k0s-generated admin cert with the
		// Prometheus sidecar, see k0sDataVolume.
		if !volumeNeeded {
			break
		}
		statefulSet.Spec.Template.Spec.Volumes = append(statefulSet.Spec.Template.Spec.Volumes, v1.Volume{
...
// certsMountPath is where the projected certificate Secrets are mounted in the
// controller container. The entrypoint copies them into the k0s pki directory,
// since k0s trips over the read-only mount and its permissions.
const certsMountPath = "/certs"

// k0sDataVolume returns the volume backing the k0s data directory, the path it
// is mounted at, and whether such a volume is needed at all.
//
// The data directory is fully reconstructible on every container start: pki is
// copied from Secrets by the entrypoint, the binaries ship pre-unpacked in the
// k0s image and manifests are mounted as ConfigMap/Secret volumes. Cluster data
// always lives outside the pod (the etcd StatefulSet, an external kine
// datasource, or the nats-data PVC). So by default no volume is mounted at all.
//
// The exception is the Prometheus sidecar, which scrapes the control plane
// using /var/lib/k0s/pki/admin.crt. That cert is generated by k0s at runtime
// and exists in no Secret, so sharing it across containers needs a volume. Only
// pki is covered, so that the binaries the k0s image pre-unpacks into
// /var/lib/k0s/bin (with cap_net_bind_service already set on kube-apiserver)
// aren't shadowed by the mount.
//
// The deprecated spec.persistence keeps covering the whole data directory, for
// clusters that still rely on files written into it out of band.
func k0sDataVolume(kmc *km.Cluster) (name, mountPath string, needed bool) {
	switch kmc.Spec.Persistence.Type {
	case "hostPath":
		return kmc.GetVolumeName(), "/var/lib/k0s", true
	case "pvc":
		name := kmc.GetVolumeName()
		if kmc.Spec.Persistence.PersistentVolumeClaim != nil && kmc.Spec.Persistence.PersistentVolumeClaim.Name != "" {
			name = kmc.Spec.Persistence.PersistentVolumeClaim.Name
		}
		return name, "/var/lib/k0s", true
	default: // emptyDir
		if !kmc.Spec.Monitoring.Enabled {
			return "", "", false
		}
		return kmc.GetVolumeName(), "/var/lib/k0s/pki", true
	}
}

// mountSecrets mounts the certificates as a projected secret volume into the
// controller container, for the entrypoint to copy into the k0s pki directory.
func mountSecrets(kmc *km.Cluster, sfs *apps.StatefulSet) {
	...
	// The entrypoint copies these into /var/lib/k0s/pki on every start, since
	// k0s trips over the permissions and RO mounts of the projected volume.
	// Doing it in the entrypoint rather than an init container means the k0s
	// data directory needs no volume of its own to carry the copy across.
	sfs.Spec.Template.Spec.Containers[0].VolumeMounts = append(sfs.Spec.Template.Spec.Containers[0].VolumeMounts, v1.VolumeMount{
		Name:      "certs",
...
func addMonitoringStack(kmc *km.Cluster, statefulSet *apps.StatefulSet) {
	...
	// Prometheus scrapes the control plane with the admin client cert that k0s
	// generates into the pki directory, so it shares the k0s data volume.
	k0sDataVolumeName, k0sDataMountPath, _ := k0sDataVolume(kmc)
