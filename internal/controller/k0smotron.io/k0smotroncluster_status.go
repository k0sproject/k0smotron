...
func translateStatusFromStatefulSet(kmc *k0smotroniov1beta2.Cluster, sts *apps.StatefulSet) {
	if sts != nil {
		...
		// use historical field logic to set the deprecated Ready field for backward compatibility where Ready is true if the number of ready
		// replicas matches the desired replicas in the spec.
		kmc.SetReadyStatus(sts.Status.ReadyReplicas == kmc.Spec.Replicas)
	}
}

func setAvailableCondition(kmc *k0smotroniov1beta2.Cluster) {
	summaryOpts := []conditions.SummaryOption{
		conditions.ForConditionTypes{
			k0smotroniov1beta2.ClusterControlPlaneFunctionalCondition,
			k0smotroniov1beta2.ClusterKubeconfigSecretAvailableCondition,
			k0smotroniov1beta2.ClusterControlPlaneExposedCondition,
		},
		// Instruct summary to consider Deleting condition with negative polarity.
		conditions.NegativePolarityConditionTypes{k0smotroniov1beta2.ClusterDeletingCondition},
	}
	availableCondition, err := conditions.NewSummaryCondition(kmc, k0smotroniov1beta2.ClusterAvailableCondition, summaryOpts...)
	if err != nil {
		log.Error(err, "Failed to set Available condition")
		availableCondition = &metav1.Condition{
			Type:    k0smotroniov1beta2.ClusterAvailableCondition,
			Status:  metav1.ConditionUnknown,
			Reason:  k0smotroniov1beta2.InternalErrorReason,
...
func setControlPlaneKubeconfigAvailableCondition(kmc *k0smotroniov1beta2.Cluster, controlPlaneState controlplaneState) {
	...
	if controlPlaneState.kubeconfig.message == "" {
		if controlPlaneState.sts == nil {
			...
			reason = k0smotroniov1beta2.ClusterControlPlaneNoReadyReplicasReason
		} else {
	...
	if controlPlaneState.kubeconfig.data == nil {
		conditions.Set(kmc, metav1.Condition{
			Type:    k0smotroniov1beta2.ClusterKubeconfigSecretAvailableCondition,
			Status:  metav1.ConditionFalse,
			Reason:  reason,
...
func setControlPlaneExposedCondition(kmc *k0smotroniov1beta2.Cluster, svc *corev1.Service) {
	...
	if kmc.Spec.ExternalAddress != "" {
		conditions.Set(kmc, metav1.Condition{
			Type:   k0smotroniov1beta2.ClusterControlPlaneExposedCondition,
			Status: metav1.ConditionTrue,
			Reason: k0smotroniov1beta2.ClusterControlPlaneExposedReason,
		})
	} else {
		conditions.Set(kmc, metav1.Condition{
			Type:    k0smotroniov1beta2.ClusterControlPlaneExposedCondition,
...
func setControlPlaneScalingCondition(kmc *k0smotroniov1beta2.Cluster, sts *apps.StatefulSet) {
	if sts == nil {
		conditions.Set(kmc, metav1.Condition{
			Type:    k0smotroniov1beta2.ClusterControlPlaneScalingCondition,
			Status:  metav1.ConditionFalse,
			Reason:  k0smotroniov1beta2.NotFoundReason,
			Message: "Check controller logs for more details",
		})
	...
	if kmc.Spec.Replicas > sts.Status.Replicas {
		conditions.Set(kmc, metav1.Condition{
			Type:    k0smotroniov1beta2.ClusterControlPlaneScalingCondition,
			Status:  metav1.ConditionTrue,
			Reason:  k0smotroniov1beta2.ClusterControlPlaneScaledUpReason,
	...
	if sts.Status.Replicas > kmc.Spec.Replicas {
		conditions.Set(kmc, metav1.Condition{
			Type:    k0smotroniov1beta2.ClusterControlPlaneScalingCondition,
			Status:  metav1.ConditionTrue,
			Reason:  k0smotroniov1beta2.ClusterControlPlaneScaledDownReason,
	...
	conditions.Set(kmc, metav1.Condition{
		Type:   k0smotroniov1beta2.ClusterControlPlaneScalingCondition,
		Status: metav1.ConditionFalse,
		Reason: k0smotroniov1beta2.ClusterControlPlaneNotScalingReason,
	})
}

func setControlPlaneUpToDateCondition(kmc *k0smotroniov1beta2.Cluster, sts *apps.StatefulSet) {
	if sts == nil {
		conditions.Set(kmc, metav1.Condition{
			Type:    k0smotroniov1beta2.ClusterControlPlaneUpToDateCondition,
			Status:  metav1.ConditionFalse,
			Reason:  k0smotroniov1beta2.NotFoundReason,
			Message: "Check controller logs for more details",
		})
	...
	if kmc.Spec.Replicas != sts.Status.UpdatedReplicas {
		conditions.Set(kmc, metav1.Condition{
			Type:    k0smotroniov1beta2.ClusterControlPlaneUpToDateCondition,
			Status:  metav1.ConditionFalse,
			Reason:  k0smotroniov1beta2.ClusterControlPlaneNotAllReplicasUpToDateReason,
		...
		conditions.Set(kmc, metav1.Condition{
			Type:   k0smotroniov1beta2.ClusterControlPlaneUpToDateCondition,
			Status: metav1.ConditionTrue,
			Reason: k0smotroniov1beta2.ClusterControlPlaneReplicasUpToDateReason,
		})
	}
}

func setControlPlaneFunctionalCondition(ctx context.Context, c client.Client, kmc *k0smotroniov1beta2.Cluster, sts *apps.StatefulSet) {
	if sts == nil {
		conditions.Set(kmc, metav1.Condition{
	...
	if !conditions.IsTrue(kmc, k0smotroniov1beta2.ClusterKubeconfigSecretAvailableCondition) {
		conditions.Set(kmc, metav1.Condition{
			Type:    k0smotroniov1beta2.ClusterControlPlaneFunctionalCondition,
			Status:  metav1.ConditionFalse,
			Reason:  k0smotroniov1beta2.ClusterControlPlaneFunctionalReason,
...
func setDeletingCondition(kmc *k0smotroniov1beta2.Cluster, reason, message string) {
	if kmc.DeletionTimestamp.IsZero() {
		conditions.Set(kmc, metav1.Condition{
			Type:   k0smotroniov1beta2.ClusterDeletingCondition,
			Status: metav1.ConditionFalse,
			Reason: k0smotroniov1beta2.ClusterNotDeletingReason,
		})
		return
	}
