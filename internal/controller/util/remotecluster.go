...
func GetKmcClientFromClusterKubeconfigSecret(ctx context.Context, hubClient client.Client, remoteClusterAccesor *kapi.RemoteHostCluster, accessCfg *access.Config) (client.Client, *kubernetes.Clientset, *rest.Config, error) {
	...
	c, err := client.New(restConfig, client.Options{Scheme: hubClient.Scheme()})
	if err != nil {
...
func getRemoteClusterClientRESTConfig(ctx context.Context, hubClient client.Client, remoteClusterAccesor *kapi.RemoteHostCluster, accessCfg *access.Config) (*rest.Config, error) {
	...
	return nil, fmt.Errorf("either kubeconfigRef or clusterProfileRef must be set to access remote cluster")
}

func restConfigFromKubeconfigRef(ctx context.Context, hubClient client.Client, kubeconfigRef *kapi.KubeconfigRef) (*rest.Config, error) {
	kubeconfigSecret := &corev1.Secret{}
	key := client.ObjectKey{
		Namespace: kubeconfigRef.Namespace,
		Name:      kubeconfigRef.Name,
	}
	err := hubClient.Get(ctx, key, kubeconfigSecret)
	if err != nil {
	...
	if !ok {
		return nil, fmt.Errorf("missing %s key in secret %s/%s", kubeconfigRef.Key, kubeconfigSecret.Namespace, kubeconfigSecret.Name)
	}
