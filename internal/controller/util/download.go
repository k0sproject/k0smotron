...
func DownloadCommands(preInstalledK0s bool, downloadURL string, version string, k0sInstallPath string) ([]string, error) {
	...
	if downloadURL != "" {
		...
		case "https", "http":
			return []string{
				fmt.Sprintf("curl -sSfL --retry 5 %s -o /usr/local/bin/k0s", downloadURL),
				"chmod +x /usr/local/bin/k0s",
			}, nil
		case "oci":
			k0sBinPath := fmt.Sprintf("%s/k0s", k0sInstallPath)
	...
	if k0sInstallPath != "" {
		scriptVars = append(scriptVars, fmt.Sprintf("K0S_INSTALL_PATH=%s", k0sInstallPath))
	}
