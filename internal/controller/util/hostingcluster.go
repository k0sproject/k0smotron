...
// CredentialProvidersConfigPath is the path to the credential providers config file.
// Set by cmd/main.go from the --credential-providers-config flag.
...
// It uses the credentials.BuildConfigFromCP to build a rest.Config with ExecProvider-based authentication.
func GetKmcClientFromClusterProfile(ctx context.Context, managementClusterClient client.Client, clusterProfileRef *kapi.ClusterProfileRef) (client.Client, *kubernetes.Clientset, *rest.Config, error) {
	...
	if CredentialProvidersConfigPath == "" {
		return nil, nil, nil, fmt.Errorf("credential providers config path is not set; use --credential-providers-config flag")
	}

	credProvider, err := credentials.NewFromFile(CredentialProvidersConfigPath)
	if err != nil {
		return nil, nil, nil, fmt.Errorf("failed to load credential providers config: %w", err)
	}

	restConfig, err := credProvider.BuildConfigFromCP(cp)
	if err != nil {
