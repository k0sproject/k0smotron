...
func (c *K0sController) reconcileInplaceK0sVersionUpdate(ctx context.Context, scope *controlplane) (ctrl.Result, error) {
	...
	// Ensure that target machines have the target version. Update version go routine may have been stopped
	...
	err = c.ensureTargetMachinesWithTargetVersion(ctx, scope.activeMachines, plan)
	if err != nil {
...
func (c *K0sController) ensureTargetMachinesWithTargetVersion(ctx context.Context, activeMachines collections.Machines, plan *unstructured.Unstructured) error {
	targetNodes, err := autopilot.GetPlanTargetNodes(plan)
	if err != nil {
		return fmt.Errorf("error getting plan target nodes: %w", err)
	}

	targetVersion, err := autopilot.GetPlanTargetVersion(plan)
	if err != nil {
		return fmt.Errorf("error getting plan target version: %w", err)
	}

	for _, nodeStr := range targetNodes {
		machine, ok := activeMachines[nodeStr]
		if !ok {
			// Plan might target a worker node, which is not part of the control plance controller scope, so we can skip it.
			continue
		}
