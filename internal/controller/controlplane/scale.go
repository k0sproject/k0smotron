...
func (c *K0sController) preflightChecks(ctx context.Context, scope *controlplane) (ctrl.Result, error) {
	...
	// Before machines scaling, we need to make sure that all deleted machines are fully deleted and their k0s resources cleaned
	// up. This is needed to ensure a graceful deletion of the control plane machines when the user deletes them directly, without
	// relying on the k0smotron controller.
	err = c.reconcileDeletedMachines(ctx, scope)
	if err != nil {
		return ctrl.Result{}, fmt.Errorf("error reconciling deleted machines: %w", err)
	}
...
func (c *K0sController) inplaceUpdate(ctx context.Context, scope *controlplane) error {
	kubeClient, err := c.getKubeClient(ctx, scope.cluster)
	if err != nil {
...
func (c *K0sController) reconcileDeletedMachines(ctx context.Context, scope *controlplane) error {
