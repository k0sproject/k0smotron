...
func (c *K0sController) createMachineFromTemplate(ctx context.Context, name string, cluster *clusterv1.Cluster, kcp *cpv1beta1.K0sControlPlane) (*unstructured.Unstructured, error) {
	...
	_, err = c.DynamicClient.Resource(gvr).Namespace(machineFromTemplate.GetNamespace()).Patch(
		context.TODO(),
		name,
		types.MergePatchType, // Use MergePatch to only modify specified fields
		patchBytes,
		metav1.PatchOptions{FieldManager: "k0smotron"},
	)

	if err != nil {
		return nil, fmt.Errorf("error merge patching: %w", err)
	}
	//return machineFromTemplate, nil
