...
func (c *K0sController) Reconcile(ctx context.Context, req ctrl.Request) (res ctrl.Result, err error) {
	...
	c.reconcileCertificateConditions(ctx, controlplane)
...
// reconcileCertificateConditions reports CA expiry on the K0sControlPlane.
// Leaf certificates on K0sControlPlane machines are generated by k0s on the
// machine itself and are not visible from the management cluster, so only
// the CAs can be reported here.
func (c *K0sController) reconcileCertificateConditions(ctx context.Context, controlplane *controlplane) {
	infos, unreadable := inspectClusterCertificates(ctx, c.Client,
		capiutil.ObjectKey(controlplane.cluster))
