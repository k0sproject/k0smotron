...
func (c *K0sController) reconcileMachines(ctx context.Context, cluster *clusterv1.Cluster, kcp *cpv1beta1.K0sControlPlane) error {
	...
	if clusterIsUpdating {
		...
		if kcp.Spec.UpdateStrategy == cpv1beta1.UpdateRecreate {
			...
			if err != nil {
				log.Log.Info("error checking CAPI version suitability for in-place updates", "error", err)
			}

			isK0smotronExtensionForInplaceUpdateDeployed, err := isK0smotronExtensionForInplaceUpdateDeployed(ctx, c.Client)
			if err != nil {
				log.Log.Info("error checking if k0smotron extension for in-place updates is deployed", "error", err)
			}
...
func isCAPIVersionSuitableForInplaceUpdate(ctx context.Context, c client.Client) (bool, error) {
	capiProviders := &clusterctlv1.ProviderList{}
	err := c.List(ctx, capiProviders,
		client.MatchingLabels{
			// Per CAPI convention, the provider SHOULD be labeled with the clusterctl provider name to allow identifying it among other providers.
			clusterv1.ProviderNameLabel: clusterctlconfig.ClusterAPIProviderName,
		},
	)
	if err != nil {
		return false, fmt.Errorf("error listing CAPI providers: %w", err)
	}
	if len(capiProviders.Items) != 1 {
		return false, fmt.Errorf("unexpected number of CAPI providers found: %d", len(capiProviders.Items))
	}

	capiVersionInstalled, err := version.NewVersion(capiProviders.Items[0].Version)
	if err != nil {
