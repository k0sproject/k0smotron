...
func (c *K0sController) reconcileKubeconfig(ctx context.Context, controlplane *controlplane) error {
	...
	if err != nil {
		if apierrors.IsNotFound(err) {
			...
			return c.createKubeconfigSecret(ctx, kc, controlplane.cluster, secret.Name(controlplane.cluster.Name, secret.Kubeconfig), controlplane.kcp.Spec.KubeconfigSecretMetadata)
		}
		...
		if controlplane.kcp.Spec.K0sConfigSpec.Tunneling.Mode == "proxy" {
			...
			if err != nil {
				if apierrors.IsNotFound(err) {
					...
					err = c.createKubeconfigSecret(ctx, kc, controlplane.cluster, secretName, controlplane.kcp.Spec.KubeconfigSecretMetadata)
					if err != nil {
						return err
					}
				}
...
func (c *K0sController) reconcileConfig(ctx context.Context, controlplane *controlplane) error {
	...
	if controlplane.kcp.Spec.K0sConfigSpec.K0s != nil {
		...
		dErr := kutil.ReconcileDynamicConfig(ctx, client.ObjectKeyFromObject(controlplane.cluster), c.Client, *controlplane.kcp.Spec.K0sConfigSpec.K0s.DeepCopy(), controlplane.kcp)
		if dErr != nil {
