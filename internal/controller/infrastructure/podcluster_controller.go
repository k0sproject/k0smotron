...
const (
	// PodClusterFinalizer is the finalizer used by the PodCluster controller.
...
// Reconcile reconciles the PodCluster resource and ensures it is in a ready state.
func (r *PodClusterController) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	c := &infrastructurev1beta2.PodCluster{}
	...
	if c.ObjectMeta.DeletionTimestamp.IsZero() {
		...
		if err := r.Status().Update(ctx, c); err != nil {
			logger.Error(err, "Failed to update PodCluster status")
			return ctrl.Result{}, err
		}
	}
...
func (r *PodClusterController) SetupWithManager(mgr ctrl.Manager, opts controller.Options) error {
	return ctrl.NewControllerManagedBy(mgr).
		WithOptions(opts).
		For(&infrastructurev1beta2.PodCluster{}).
		Owns(&corev1.Pod{}).
		Complete(r)
}
