...
import (
	...
	infrastructurev1beta2 "github.com/k0sproject/k0smotron/v2/api/infrastructure/v1beta2"
...
func (r *PodMachineController) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	podMachine := &infrastructurev1beta2.PodMachine{}
...
func (r *PodMachineController) reconcileDelete(ctx context.Context, podMachine *infrastructurev1beta2.PodMachine) (ctrl.Result, error) {
...
func (r *PodMachineController) reconcileNormal(ctx context.Context, podMachine *infrastructurev1beta2.PodMachine, machine *clusterv1.Machine) (ctrl.Result, error) {
	logger := log.FromContext(ctx).WithValues("podmachine", types.NamespacedName{Name: podMachine.Name, Namespace: podMachine.Namespace})

	// The patch helper snapshots the object on creation, so it must be created before
	// any modification of the PodMachine below, otherwise the changes are not part of the patch.
	...
	// ProviderID is part of the Cluster API contract: the Machine controller waits for the
	// infrastructure provider to set it before considering the machine provisioned.
	podMachine.Spec.ProviderID = fmt.Sprintf("pod-machine://%s/%s", pod.Namespace, pod.Name)

	r.updateStatus(podMachine, pod)

	if err := patchHelper.Patch(ctx, podMachine); err != nil {
		logger.Error(err, "Failed to patch PodMachine")
		return ctrl.Result{}, err
	}
...
func (r *PodMachineController) createOrUpdatePod(ctx context.Context, podMachine *infrastructurev1beta2.PodMachine, machine *clusterv1.Machine) (*corev1.Pod, error) {
...
func (r *PodMachineController) createCloudInitConfigMap(ctx context.Context, podMachine *infrastructurev1beta2.PodMachine) (*corev1.ConfigMap, error) {
...
func (r *PodMachineController) addCloudInitVolumeMounts(pod *corev1.Pod, podMachine *infrastructurev1beta2.PodMachine) {
	configMapName := podMachine.Name
...
// updateStatus updates the PodMachine status in memory to reflect the state of the given pod.
// Persisting the changes is the caller's responsibility.
func (r *PodMachineController) updateStatus(podMachine *infrastructurev1beta2.PodMachine, pod *corev1.Pod) {
	...
	// provisioned is part of the CAPI contract and must not flip back to false once set,
	// so only latch it to true. Current readiness is reported via conditions.
	if ready {
		podMachine.Status.Initialization.Provisioned = new(true)
	}

	readyCondition := metav1.Condition{
		Type:   infrastructurev1beta2.PodMachineReadyCondition,
		Status: metav1.ConditionFalse,
		Reason: infrastructurev1beta2.PodMachineNotReadyReason,
	}
	if ready {
		readyCondition.Status = metav1.ConditionTrue
		readyCondition.Reason = infrastructurev1beta2.PodMachineReadyReason
	}
	conditions.Set(podMachine, readyCondition)
}
...
func (r *PodMachineController) SetupWithManager(mgr ctrl.Manager, opts capictrl.Options) error {
	return ctrl.NewControllerManagedBy(mgr).
		WithOptions(opts).
		For(&infrastructurev1beta2.PodMachine{}).
		Owns(&corev1.Pod{}).
		Complete(r)
}
