...
func (s *HAControllerNatsSuite) TestK0sGetsUp() {
	...
	s.T().Log("deploying k0smotron cluster with NATS storage")
	s.createK0smotronCluster(s.Context(), kc)
	s.Require().NoError(common.WaitForStatefulSet(s.Context(), kc, "kmc-kmc-test", "kmc-test"))

	s.T().Log("verifying NATS headless service was created")
	_, err = kc.CoreV1().Services("kmc-test").Get(s.Context(), "kmc-kmc-test-nats", metav1.GetOptions{})
	s.Require().NoError(err, "NATS headless service should exist")
...
func (s *HAControllerNatsSuite) createK0smotronCluster(ctx context.Context, kc *kubernetes.Clientset) {
...
// getPod returns one of the running k0smotron control-plane pods.
// With NATS storage the NATS server is embedded in kine, so there is no
// separate etcd StatefulSet: only the 3 k0s pods are expected.
func (s *HAControllerNatsSuite) getPod(ctx context.Context, kc *kubernetes.Clientset) corev1.Pod {
	...
	s.Require().Equal(3, len(pods.Items), "expected 3 kmc-test pods (NATS is embedded, no separate etcd pods), got %d", len(pods.Items))
