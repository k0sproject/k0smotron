...
func (s *CAPIControlPlaneDockerDownScalingSuite) TestCAPIControlPlaneDockerDownScaling() {
	...
	scaleDownCtx, cancel := context.WithTimeout(s.ctx, 20*time.Minute)
	defer cancel()

	// wait for the new controlplane + worker node to come up
	err = wait.PollUntilContextCancel(scaleDownCtx, 10*time.Second, true, func(ctx context.Context) (done bool, err error) {
		node, err := kmcKC.CoreV1().Nodes().List(ctx, metav1.ListOptions{})
		if err != nil {
			return false, nil
		}

		return len(node.Items) == 2, nil
	})
	s.Require().NoError(err)
