...
func (s *CAPIControlplaneHealthCheckRemediation) TestCAPIControlPlaneDockerDownScaling() {
	...
	for i := 0; i < 3; i++ {
		...
		err = wait.PollImmediateUntilWithContext(s.ctx, 1*time.Second, func(ctx context.Context) (bool, error) {
			...
			// condition to check that new machine is generated because previous had 'cluster.x-k8s.io/remediate-machine' annotation
			hasForceRemediationAnnotation := false
			if i == 0 {
				hasForceRemediationAnnotation = s.hasForceRemediationAnnotation()
			}

			return containsVersion && !hasForceRemediationAnnotation, nil
		})
		s.Require().NoError(err)
	}
}
...
func (s *CAPIControlplaneHealthCheckRemediation) hasForceRemediationAnnotation() bool {
	// Exec via kubectl
	out, err := exec.Command(
		"sh",
		"-c",
