...
type K0sWorkerConfigSpec struct {
	...
	// WorkingDir specifies the working directory where k0smotron will place its files and execute commands.
...
// GetDefaultK0sConfigPath returns the full path to the k0s.yaml file in the working directory.
func (kcs *K0sConfigSpec) GetDefaultK0sConfigPath() string {
	if kcs.WorkingDir == "" {
		return "/etc/k0s.yaml"
	}
...
// GetDefaultK0sConfigPath returns the full path to the k0s.yaml file in the working directory.
func (k *K0sWorkerConfig) GetDefaultK0sConfigPath() string {
