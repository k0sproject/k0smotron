...
func init() {
	SchemeBuilder.Register(&K0sConfig{}, &K0sConfigList{})
}

// +kubebuilder:object:root=true

// K0sConfigList contains a list of K0sConfig.
type K0sConfigList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitempty"`
	Items           []K0sConfig `json:"items"`
}
...
// K0sConfig is the Schema for the k0sconfigs API.
type K0sConfig struct {
	...
	// spec is the desired state of K0sConfig.
	// +optional
	Spec K0sConfigSpec `json:"spec,omitempty"`
	// status is the observed state of K0sConfig.
	// +optional
	Status K0sConfigStatus `json:"status,omitempty"`
}
...
type K0sConfigSpec struct {
	...
	ClusterConfig *unstructured.Unstructured `json:"clusterConfig,omitempty"`
...
// K0sConfigStatus defines the observed state of K0sConfig
type K0sConfigStatus struct {
...
func (c *K0sConfig) GetConditions() []metav1.Condition {
	return c.Status.Conditions
}

// SetConditions sets the conditions on the K0sConfig status.
func (c *K0sConfig) SetConditions(conditions []metav1.Condition) {
	c.Status.Conditions = conditions
}

// GetK0sConfigPath returns the full path to the k0s.yaml file in the working directory.
func (cs *K0sConfigSpec) GetK0sConfigPath() string {
	if cs.WorkingDir == "" {
		return "/etc/k0s.yaml"
	}
	return filepath.Join(cs.WorkingDir, "k0s.yaml")
}

// GetJoinTokenPath returns the full path to the k0s token file in the working directory.
func (cs *K0sConfigSpec) GetJoinTokenPath() string {
	if cs.WorkingDir == "" {
		return "/etc/k0s.token"
	}
	return filepath.Join(cs.WorkingDir, "k0s.token")
}

// Validate validates the K0sWorkerConfigSpec.
func (cs *K0sConfigSpec) Validate(pathPrefix *field.Path) field.ErrorList {
...
func (cs *K0sConfigSpec) validateFiles(pathPrefix *field.Path) field.ErrorList {
	var allErrs field.ErrorList
...
func (cs *K0sConfigSpec) validateVersion(pathPrefix *field.Path) field.ErrorList {
	var allErrs field.ErrorList
