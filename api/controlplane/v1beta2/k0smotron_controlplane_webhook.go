...
func (v *K0smotronControlPlaneValidator) ValidateCreate(_ context.Context, c *K0smotronControlPlane) (admission.Warnings, error) {
	if c == nil {
		return nil, fmt.Errorf("expected a K0smotronControlPlane object but got nil")
	}

	warnings, err := v.validate(c)
	if err != nil {
		return warnings, err
	}
...
func (v *K0smotronControlPlaneValidator) ValidateUpdate(_ context.Context, oldObj, newObj *K0smotronControlPlane) (admission.Warnings, error) {
	newKCP := newObj
	oldKCP := oldObj
